...
public class DatabaseHandler {
     ...
     * @throws SQLException           If there is a problem reading from the
     *                                database.
     */
    ...
    public Map<String, String> readProductTypes()
        ...
        while (resultSet.next()) {
            ...
            String typeProduct = resultSet.getString("typeDescription").trim();
            types.put(type, typeProduct);
        }
        resultSet.close();
        statement.close();
    ...
    private Warehouse readWarehouse(Connection connection) throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            Location theLocation = new Location(Integer.parseInt(rowCol[0]),
    ...
    private ClientOrders readClientOrders(Connection connection)
        ...
        ResultSet resultSet = statement.executeQuery(
                "select * from clientOrders, orderItems where clientOrders.orderNumber = orderItems.orderNumber order by clientOrders.orderNumber");
        ...
        while (resultSet.next()) {
            ...
            if (orderNumber != previousOrder) {
                ...
                ClientOrder anOrder = new ClientOrder(orderNumber, customerCode,
    ...
    private PurchaseOrders readPurchaseOrders(Connection connection)
        ...
        Statement statement = connection.createStatement();
        ResultSet resultSet = statement.executeQuery(
                "select * from purchaseOrders, purchaseOrderItems where purchaseOrders.purchaseOrderNumber = purchaseOrderItems.purchaseOrderNumber order by purchaseOrders.purchaseOrderNumber");
        ItemInventory itemInventory = null;
    ...
    private Deliveries readDeliveries(Connection connection)
        ...
        Statement statement = connection.createStatement();
        ResultSet resultSet = statement.executeQuery(
                "select * from deliveries, deliveryItems where deliveries.deliveryNumber = deliveryItems.deliveryNumber order by deliveries.deliveryNumber");
        ItemInventory itemInventory = null;
        ...
        while (resultSet.next()) {
            ...
            if (deliveryNumber != previousDelivery) {
                // A new order.
                itemInventory = new ItemInventory();
                Delivery aDelivery = new Delivery(deliveryNumber, new Date(dateDelivered),
